...
export const AdminApplicantsSection: React.FC<AdminApplicantsSectionProps> = ({
  ...
  const filteredApplicants = useMemo(() => {
    return applicants.filter((applicant) => {
      ...
      const matchesRating =
        filters.rating === 'all' || applicant.rating >= parseInt(filters.rating);

      const matchesStage =
        filters.stage === 'all' || applicant.stage === filters.stage;
              ...
              <div className="flex justify-center mb-2">
                {[...Array(5)].map((_, i) => (
                  <svg
                    key={i}
                    className={`w-4 h-4 ${
                      i < applicant.rating ? 'text-yellow-400' : 'text-gray-300'
                    }`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
