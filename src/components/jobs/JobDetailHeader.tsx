...
export const JobDetailHeader: React.FC<JobDetailHeaderProps> = ({ job, isLoading = false }) => {
  ...
  const title = job?.title || (isLoading ? 'Cargando...' : 'Cargo sin título');
  const location = job?.location || 'Ubicación no especificada';
  const categoryName = job?.category?.name || 'Categoría no especificada';
  const jobType = (job?.job_type && jobTypeLabels[job.job_type]) || 'Modalidad no especificada';

  const updatedLabel = useMemo(() => {
    const dateString = job?.published_at ?? job?.updated_at ?? job?.created_at;
    if (!dateString) {
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center gap-3 md:gap-4 lg:gap-6 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className={colorClasses.text.gray600}>{location}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={colorClasses.text.gray600}>{categoryName}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={colorClasses.text.gray600}>{jobType}</span>
            </div>
          </div>
